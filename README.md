...
<p align="center">
  Fork of <code>every-code/main</code> with a focused set of changes (Android/Termux builds, Codex ports into <code>code-rs</code>, managed network mediation, a context-aware Memories pipeline, app-server exec v2 with Windows sandboxing, and TUI UX cleanup).
</p>
...
| Session context mode / auto-context              | Hybrid      | Session tuning exposes explicit context modes plus auto-context checks. On supported models, `auto` uses the 1M-capable limits while keeping pre-turn compaction checks active; `enabled` forces the same large window without the extra “when should I compact?” explanation layer, and `disabled` stays on the model-family defaults. | `code-rs/core/src/protocol.rs`<br>`code-rs/core/src/codex/streaming/submission/configure_session/*`<br>`code-rs/tui/src/bottom_pane/model_selection_view.rs` |
...
- Utility backports: `stream-parser`, `sleep-inhibitor`, plus newer `pty`/`string` helpers that support exec/runtime parity work.
...
- Session context mode and auto-context checks are wired through the local configure-session path and TUI tuning flow. In practice, `auto` is the “1M with guardrails” mode on supported models: it enables the extended window, keeps the auto-context check events flowing, and warns or compacts before later turns when context pressure gets too high.
- Memories use upstream-style extraction/selection ideas, but this fork keeps `SessionCatalog` authoritative and publishes immutable snapshot generations.
