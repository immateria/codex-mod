...
const SYSTEM_SKILLS_DIR: Dir = include_dir::include_dir!(
    "$CARGO_MANIFEST_DIR/src/assets/system_skills"
);
...
///
/// This is typically located at `CODE_HOME/skills/.system`.
...
/// Installs embedded system skills into `CODE_HOME/skills/.system`.
///
...
/// install is skipped.
pub fn install_system_skills(code_home: &Path) -> Result<(), SystemSkillsError> {
    let code_home = AbsolutePathBuf::try_from(code_home)
        .map_err(|source| SystemSkillsError::io("normalize code home dir", source))?;

    let skills_root_dir = code_home
        .join(SKILLS_DIR_NAME)
    ...
    let dest_system = system_cache_root_dir_abs(&code_home)
