...
pub fn is_dangerous_command_windows(command: &[String]) -> bool {
    if is_dangerous_windows_token_sequence(command) {
        return true;
    }
...
/// Detect dangerous Windows-oriented operations from a flat token sequence,
/// regardless of which shell produced those tokens.
///
/// This makes safety checks work for alternate shells (for example Nushell or
/// Elvish) running on Windows where command names/flags still map to dangerous
/// operations.
pub fn is_dangerous_windows_token_sequence(command: &[String]) -> bool {
    let Some(first_raw) = command.first() else {
        return false;
    };

    let tokens_lc: Vec<String> = command
        .iter()
        .map(|token| {
            token
                .trim_matches('\'')
    ...
    if (first == "del" || first == "erase") && has_force_flag_cmd(command) {
        return true;
    }

    if (first == "rd" || first == "rmdir")
        && has_recursive_flag_cmd(command)
        && has_quiet_flag_cmd(command)
    {
        return true;
    }

    let has_url = args_have_url(command);
    if has_url
        && matches!(
            first.as_str(),
            "start-process"
    ...
    if has_url && is_browser_executable(first.as_str()) {
        return true;
    }

    if (first == "rundll32" || first == "rundll32.exe")
        && tokens_lc
            .iter()
