    ...
    pub(super) fn freeze_history_cell_at(&mut self, idx: usize, render_settings: RenderSettings) -> bool {
        ...
        let frozen = FrozenHistoryCell::new(
            *history_id,
            ...
            cached_height,
            cell.call_id().map(ToString::to_string),
            cell.parent_call_id().map(ToString::to_string),
        );
