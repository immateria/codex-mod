...
#[inline]
pub(crate) fn handle_rate_limit(_event: &RateLimitSnapshotEvent, _sender: &AppEventSender) {
...
#[inline]
pub(crate) fn handle_browser_screenshot(_event: &BrowserScreenshotUpdateEvent, _sender: &AppEventSender) {
    // Intentionally no-op: downstream wrappers can forward to AppEvent if needed.
}
