...
impl<'a> SettingsSectionedPanel<'a> {
    ...
    fn layout_from_content(&self, content: Rect) -> Option<SettingsSectionedPanelLayout> {
        ...
        let header_h = u16::try_from(self.header_rows)
            .unwrap_or(u16::MAX)
            .min(content.height);
        let min_body = u16::try_from(self.min_body_rows)
            .unwrap_or(u16::MAX)
            .max(1);

        let available = content.height.saturating_sub(header_h);
        let footer_h = u16::try_from(self.footer_rows)
            .unwrap_or(u16::MAX)
            .min(available);
        let body_h = available.saturating_sub(footer_h);
        ...
        let footer_y = body_y.saturating_add(body_h);
...
mod tests {
    ...
    fn render_produces_expected_section_rects() {
        ...
        assert_eq!(rendered.body, Rect::new(2, 2, 36, 6));
        assert_eq!(rendered.footer, Rect::new(2, 8, 36, 1));
    }
    ...
    fn margin_applies_before_section_split() {
        ...
        assert_eq!(layout.body, Rect::new(2, 2, 16, 4));
        assert_eq!(layout.footer, Rect::new(2, 6, 16, 1));
    }
