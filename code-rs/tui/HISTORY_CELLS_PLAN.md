...
## Cell Inventory
Status legend: [done] complete (semantic deterministic state ready), [wip] still needs semantic refactor.

### [done] Plain Messages – `PlainHistoryCell`
- **Desired state:**
  ```rust
  ...
  ```
- **Current status:** [done] state now stored as `PlainMessageState` (header + `Vec<MessageLine>`); follow-up: richer `MessageSegment` types for code/bullets plus metadata serialization.

### [done] Wait Status – `WaitStatusCell`
- **Desired state:** `WaitStatusState { header: WaitHeader, bullet_points: Vec<WaitDetail> }`
- **Status:** [done] migrated to `WaitStatusState` with explicit header/detail tones; renderer rebuilds lines from structured data.

### [done] Loading Spinner – `LoadingCell`
- **State:** `LoadingCellState { message: String }`
- **External settings:** theme.

### [done] Tool Calls – `ToolCallCell`
- **Desired state:**
  ```rust
  ...
  ```
- **Status:** [done] constructors now populate `ToolArgument`/`ToolResultPreview`; follow-up: polish preview truncation + metadata serialization.
- **External settings:** theme, width.

### [done] Running Tool Calls – `RunningToolCallCell`
- **Desired state:** same struct but `arguments: Vec<ToolArgument>` instead of rendered lines.
- **Status:** [done] running tool state stores `Vec<ToolArgument>` with wait caps/call ids tracked separately.
- **External settings:** theme, width, current time (for elapsed label).

### [done] Plan Updates – `PlanUpdateCell`
- **Desired state:** `PlanUpdateState { icon: PlanIcon, items: Vec<PlanLine>, completion: PlanCompletion }`
- **Status:** [done] stores `PlanUpdateState` with `PlanProgress`/`PlanStep`; follow-up: richer icons + metadata serialization.
- **External settings:** theme, width.

### [done] Upgrade Notices – `UpgradeNoticeCell`
- **Desired state:** `UpgradeNoticeState { current_version: String, latest_version: String, message: UpgradeMessage }`
- **Status:** [done] upgrade notices now keep `{ current_version, latest_version, message }`; follow-up: wire optional CTA metadata.
- **External settings:** theme, width.

### [done] Background Events – `BackgroundEventCell`
- **Status:** [done] already semantic (metadata only).
- **External settings:** theme; renderer currently emits metadata text.

### [done] Animated Welcome – `AnimatedWelcomeCell`
- **Persistence:** not stored in the state vector (UI-only animation).
- **Runtime state:** start time, fade progress, cached height.

### [done] Reasoning – `CollapsibleReasoningCell`
...
- **Status:** [done] state maintains `ReasoningSection` blocks with persisted summaries and typed bullet markers (2025-09-26 follow-up complete).
...
### [done] Explore Aggregations – `ExploreAggregationCell`
- **Status:** [done] already semantic (summary enums + statuses).
- **External settings:** theme, width.

### [done] Image Outputs – `ImageOutputCell`
- **Status:** [done] image records now store width, height, SHA, MIME type, and byte size in `HistoryState`; the UI renders from `ImageRecord` without private RGBA caches (2025-09-30).

### [done] Patch Summaries – `PatchSummaryCell`
- **Status:** [done] patch events now emit and hydrate `PatchRecord`; the cell renders directly from recorded metadata without internal width caches (2025-09-30).

### [done] Exec Commands – `ExecCell`
- **Status:** [done] `ExecRecord`/`HistoryDomainEvent` now drive exec lifecycle end-to-end; renderer pulls lines via `display_lines_from_record` through the shared cache.
...
- ### [done] Assistant Streaming – `StreamingContentCell`
...
- **Decision:** finish documenting the state-driven flow and mop up any lingering fallback usage before flipping status to [done].
...
### [done] Assistant Answers – `AssistantMarkdownCell`
...
### [done] Merged Exec Summary – `MergedExecCell`
- **Status:** [done] merged summaries now hydrate from `MergedExecRecord` entries in `HistoryState` rather than cached `ExecCell` output.
...
### [done] Diffs – `DiffCell`
- **Status:** [done] diff cells now render directly from `DiffRecord` via the shared renderer cache; the per-width RefCell cache was deleted.
...
### [done] Explore Fetch / HTTP – `ExploreRecord`
- **Status:** [done] explore aggregations now render from `ExploreRecord` state via the shared renderer cache; the legacy trailing flag and UI-owned line cache were removed.
...
### [done] Legacy Plain Producers
- **Status:** [done] plain system/user notices now build `PlainMessageState` and flow through `HistoryState`; `PlainHistoryCell::new`/prompt helpers no longer leak UI caches.
...
1.1 **Plain messages** – [done] replaced `SemanticLine` caches with `PlainMessageState` (`plain.rs` now stores `MessageHeader + MessageLine` via shared conversion helpers). Follow-up: enrich headers with structured badges and surface metadata once available.
1.2 **Tool calls (running & completed)** – [done] constructors now emit `ToolArgument`/`ToolResultPreview`; remaining work: tighten JSON summaries + result truncation heuristics.
1.3 **Plan updates** – [done] `PlanUpdateCell` now renders from `PlanProgress`/`PlanStep` + `PlanIcon`; follow-up: improved summary metadata.
1.4 **Upgrade notice** – [done] cell consumes `UpgradeNoticeState` (versions + message), custom render derived at draw time.
1.5 **Reasoning** – [done] sections/blocks stored; block metadata now includes typed bullets and per-section summaries for collapse previews (2025-09-26).
1.6 **Wait status & background notices** – [done] wait tools use `WaitStatusState`; background notices render via `BackgroundEventRecord`.
1.7 **Documentation** – [done] inline docs now cover reasoning summaries/bullets; audit of constructors confirmed strongly typed state (2025-09-26).

## Step 2 – Exec / Streaming / Diff Bridge *(In Progress)*
2.1 **Exec state extraction & module split** – [done] exec records now render exclusively via `RenderRequestKind::Exec`; tests (`exec_render_from_state`, `exec_render_remains_stable_after_insertions`) confirm history restores are state-driven with no per-cell caches.
2.2 **Streaming assistant module** – [done] streaming records hydrate from `HistoryState`; see `streaming_flow_handles_deltas_and_finalize` for start → delta → finish coverage.
2.3 **Finalized assistant markdown module** – [done] finalized answers render from `AssistantMessageState`; tests (`assistant_render_from_state`, `assistant_render_remains_stable_after_insertions`) verify state-driven output and stability.
2.4 **Diff module breakout** – [done] diff records render from state via `RenderRequestKind::Diff`; regression coverage tracks reorder stability.
2.5 **Merged exec views** – [done] merged exec summaries rebuild from `HistoryState` and share the renderer cache; no per-cell layout caches remain.

## Step 3 – HistoryState Manager *(Pending)*
3.1 **HistoryRecord enum** – [done] complete: `history/state.rs` defines `HistoryRecord`, per-cell state structs, and `HistoryState` scaffolding with ID management helpers.
3.2 **ChatWidget incremental adoption** – Introduce `HistoryState`/`HistoryRenderState` alongside the legacy `history_cells` vector, migrate low-risk cell types (plain, loading, wait status) to the new state, then remove the legacy vector once the path is stable. **Status:** [done] Wait tooling, diff inserts, and explore aggregations now flow through domain events; `history_cells` remains solely as the interim render cache until Step 3.5 removes the trait bridge.
3.3 **Apply-event pipeline** – Implement `HistoryState::apply_event(&mut self, event: &EventMsg)` covering all core/TUI event types (exec lifecycle, tool updates, background notices, resume snapshots, undo) and route migrated cells through it. **Status:** [done] Exec wait notes, diff pushes, and explore status updates now call into `HistoryState::apply_domain_event`; next wave is wiring streaming assistant/diff updates before the Step 4 renderer consolidation.
3.4 **Undo/resume hooks** – Expose `snapshot`, `restore`, and `truncate_after(id)` to support /undo and resume flows. **Status:** [done] history snapshots + truncation API landed in `HistoryState`, ghosts now capture them (2025-09-30).
...
4.1 **Centralize handlers** – Route every mutation in `handle_*` (exec events, tool deltas, diff updates, background notices) through `HistoryState` so ordering/id management lives in one place. **Status:** [done] patch approval / apply flows now issue `HistoryDomainEvent` updates (no direct cell mutations), and jump-back/ghost restore rehydrate via snapshots (2025-09-30).
4.2 **Stable IDs for streaming** – Store `HistoryId` + domain IDs (exec id, tool call id, stream id) to ensure in-place updates and dedupe. **Status:** [done] history state now tracks exec/tool call mappings, handlers resolve deltas via `HistoryId`, and regression coverage checks reordered exec output (2025-09-30).
4.3 **HistoryRenderState** – Provide adapter that consumes `HistoryRecord` + current settings (theme, width, reasoning collapsed) and produces cached `RenderedCell` structures. Cache keyed by `(HistoryId, width, theme_epoch)`. **Status:** [done] history render adapter now renders via `RenderSettings` keyed by `HistoryId`, clears caches on theme/width changes, and new tests cover caching + invalidation (2025-09-30).
4.4 **Renderer migration** – Update TUI drawing code to iterate `HistoryRenderState::visible_cells()`; drop direct use of `HistoryCell` trait. **Status:** [done] render loop now consumes `RenderRequest`/`visible_cells`, caches assistant plans, and performs layout/spacing without direct trait traversal (2025-09-30).

## Step 5 – Serialization & Persistence
- Define canonical JSON schema for each record variant (enum tags, tone names instead of colors). **Status:** [done] documented in `docs/history_state_schema.md` (2025-09-30).
- Add round-trip tests for snapshot/resume. **Status:** [done] JSON snapshot tests cover serialize/restore (`history/state.rs`) (2025-09-30).
- Persist history vector in session logs; implement `/undo` by restoring prior snapshot. **Status:** [done] session logs now emit `history_snapshot` entries and `/undo` restores `HistorySnapshot` for the forked conversation (2025-09-30).

## Step 6 – Performance Improvements *(Blocked on Step 4)*
- Ship the shared renderer cache (memoized per-cell layouts keyed by `(HistoryId, width, theme_epoch)`) so the interim exec/assistant/diff caches can be removed. **Status:** [done] history_render now caches by `(HistoryId, width, theme_epoch, reasoning)` and `ChatWidget` consumes the shared API (2025-09-30).
- Add instrumentation to measure render latency and scrolling cost pre/post refactor. **Status:** [done] perf stats now track scroll events/rows and render time; `/perf show` surfaces the new metrics (2025-09-30).
- Benchmark resume/undo operations using new snapshots. **Status:** [done] perf stats now record undo/restore timings surfaced via `/perf show` (2025-09-30).
