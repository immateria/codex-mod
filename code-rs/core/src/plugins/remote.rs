...
pub(crate) async fn fetch_remote_plugin_status(
    ...
    let token = auth.get_token().await.map_err(RemotePluginFetchError::AuthToken)?;
...
pub async fn fetch_remote_featured_plugin_ids(
    ...
    if let Some(auth) = auth.filter(|auth| auth.mode.is_chatgpt()) {
        let token = auth.get_token().await.map_err(RemotePluginFetchError::AuthToken)?;
...
async fn post_remote_plugin_mutation(
    ...
    let client = create_client(DEFAULT_ORIGINATOR);
    let token = auth.get_token().await.map_err(RemotePluginMutationError::AuthToken)?;
    let mut request = client
        .post(url.clone())
