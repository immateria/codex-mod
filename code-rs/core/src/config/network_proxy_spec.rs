...
mod imp {
    ...
    use crate::managed_network_proxy_api::ManagedNetworkProxy;
    ...
    impl StartedNetworkProxy {
        ...
        pub fn proxy(&self) -> ManagedNetworkProxy {
            // Ensure the handle stays "used" so we keep the listeners alive.
            let _ = &self.handle;
            ManagedNetworkProxy::from(self.proxy.clone())
        }
    }
...
mod imp {
    ...
    impl NetworkProxySpec {
        pub async fn start_proxy(
            ...
            _policy_decider: Option<()>,
            _blocked_request_observer: Option<()>,
            _enable_network_approval_flow: bool,
        ) -> std::io::Result<StartedNetworkProxy> {
            Err(std::io::Error::new(
                std::io::ErrorKind::Unsupported,
                "managed network mediation is not available in this build",
            ))
        }
    }
}

pub use imp::NetworkProxySpec;
pub use imp::StartedNetworkProxy;
