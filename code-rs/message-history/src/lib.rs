...
//!
//! The history is stored at `CODE_HOME/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `CODE_HOME`.
...
fn history_filepath(code_home: &Path) -> PathBuf {
    let mut path = code_home.to_path_buf();
...
pub async fn append_entry(
    ...
    // Resolve `CODE_HOME/history.jsonl` and ensure the parent directory exists.
    let path = history_filepath(code_home);
...
pub async fn history_metadata(code_home: &Path) -> (u64, usize) {
    let path = history_filepath(code_home);
...
/// locking API.
#[cfg(unix)]
pub fn lookup(log_id: u64, offset: usize, code_home: &Path) -> Option<HistoryEntry> {
    use std::io::BufRead;
