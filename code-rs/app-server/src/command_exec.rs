...
use code_app_server_protocol::CommandExecOutputDeltaNotification;
use code_app_server_protocol::CommandExecOutputStream;
use code_app_server_protocol::CommandExecResizeParams;
use code_app_server_protocol::CommandExecResizeResponse;
use code_app_server_protocol::CommandExecResponse;
use code_app_server_protocol::CommandExecTerminalSize;
use code_app_server_protocol::CommandExecTerminateParams;
use code_app_server_protocol::CommandExecTerminateResponse;
use code_app_server_protocol::CommandExecWriteParams;
use code_app_server_protocol::CommandExecWriteResponse;
use code_app_server_protocol::ServerNotification;
use code_core::bytes_to_string_smart;
use code_core::config::network_proxy_spec::StartedNetworkProxy;
use code_core::exec::DEFAULT_EXEC_COMMAND_TIMEOUT_MS;
use code_core::exec::ExecExpiration;
use code_core::exec::IO_DRAIN_TIMEOUT_MS;
use code_core::exec::SandboxType;
use code_core::sandboxing::ExecRequest;
use code_utils_pty::DEFAULT_OUTPUT_BYTES_CAP;
use code_utils_pty::ProcessHandle;
use code_utils_pty::SpawnedProcess;
use code_utils_pty::TerminalSize;
    ...
    pub(crate) async fn start(
        ...
        if matches!(exec_request.sandbox, SandboxType::WindowsRestrictedToken) {
            ...
            tokio::spawn(async move {
                ...
                match code_core::sandboxing::execute_env(exec_request, None).await {
                    Ok(output) => {
                        outgoing
        ...
        let spawned = if tty {
            code_utils_pty::spawn_pty_process(
                program,
                args,
                cwd.as_path(),
                &env,
...
mod tests {
    ...
    async fn windows_sandbox_streaming_exec_is_rejected() {
        ...
        let err = manager
            .start(StartCommandExecParams {
                outgoing: Arc::new(OutgoingMessageSender::new_with_routed_sender(tx)),
                request_id: ConnectionRequestId {
                    connection_id: ConnectionId(1),
                    request_id: mcp_types::RequestId::Integer(42),
                },
    ...
    async fn windows_sandbox_non_streaming_exec_uses_execution_path() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: mcp_types::RequestId::Integer(99),
        };
    ...
    async fn cancellation_expiration_keeps_process_alive_until_terminated() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: mcp_types::RequestId::Integer(100),
        };
        let sandbox_policy = SandboxPolicy::ReadOnly;
    ...
    async fn windows_sandbox_process_ids_reject_write_requests() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: mcp_types::RequestId::Integer(1),
        };
    ...
    async fn windows_sandbox_process_ids_reject_terminate_requests() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: mcp_types::RequestId::Integer(2),
        };
        let process_id = ConnectionProcessId {
            connection_id: request_id.connection_id,
    ...
    async fn dropped_control_request_is_reported_as_not_running() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: mcp_types::RequestId::Integer(3),
        };
