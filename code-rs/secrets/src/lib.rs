...
const KEYRING_SERVICE: &str = "Code Secrets";
...
impl SecretScope {
    ...
    pub fn canonical_key(&self, name: &SecretName) -> String {
        ...
        match self {
            ...
            Self::Environment(environment_id) => format!("env/{environment_id}/{}", name.as_str()),
        }
    }
}
...
impl fmt::Debug for SecretsManager {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        // Intentionally avoid exposing any secret material or backend internals in debug output.
...
impl SecretsManager {
    ...
    pub fn new_with_keyring_store(
        ...
        let backend: Arc<dyn SecretsBackend> = match backend_kind {
            SecretsBackendKind::Local => Arc::new(LocalSecretsBackend::new(code_home, keyring_store)),
        };
        Self { backend }
    }
...
// Tiny helper: walk upwards looking for a `.git` file or directory.
// This is intentionally lightweight and does not require `git` to be installed.
...
pub(crate) fn compute_keyring_account(code_home: &Path) -> String {
    code_keyring_store::store_key_for_code_home("secrets", code_home)
}
