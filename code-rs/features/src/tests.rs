...
use code_protocol::protocol::WarningEvent;
...
fn apps_require_feature_flag_and_chatgpt_auth() {
    ...
    let api_key_auth = code_login::CodexAuth::from_api_key("test-api-key");
    assert!(!features.apps_enabled_for_auth(Some(&api_key_auth)));

    let chatgpt_auth = code_login::CodexAuth::create_dummy_chatgpt_auth_for_testing();
